...
Candy.Core = (function (self, Strophe, $) {
	...
	var _connection = null,
		 ...
		 */ _addNamespaces = function () {
			...
			_addNamespace("DELIVERYTIME", "urn:xmpp:deliverytime");
		},
	...
	self.init = function (service, options) {
		...
		_connection = new Strophe.Connection(_service, {keepalive: true});
		_connection.rawInput = self.rawInput.bind(self);
		_connection.rawOutput = self.rawOutput.bind(self);

		//alert('_connection.xmlOutput '+JSON.stringify(_connection));
		// set caps node
		...
		// try {
		//     _connection.restore(jid, onConnect);
		// } catch(e) {
	...
	self.registerEventHandlers = function () {
		...
		self.addHandler(self.Event.Jabber.Presence, Strophe.NS.CLIENT, "presence");
		self.addHandler(self.Event.Jabber.Presence, Strophe.NS.LAST_SEEN, "iq", "result");
	...
	self.connect = function (jidOrHost, password, nick) {
		//alert('connect triggered jidOrHost='+jidOrHost+' :: password='+password+' :: nick='+nick)
		// Reset before every connection attempt to make sure reconnections work after authfail, alltabsclosed, ...
		//alert('connect from bundle disconnected jidOrHost='+jidOrHost+' ## pass'+password+' ## nick='+nick);
		_connection.reset();
		...
		//alert('connect from bundle inside connect='+Candy.Core.Event.Strophe.Connect+' ## resource'+Strophe.getResourceFromJid(jidOrHost));
	...
	self.attach = function (jid, sid, rid, nick) {
		//alert('attach Connect'+jid+' ## '+sid+' ## '+rid+' ## '+Candy.Core.Event.Strophe.Connect);
		if (nick) {
...
Candy.Util = (function (self, $) {
	...
	self.parseAndCropXhtml = function (str, len) {
		return $("<div/>")
			.append(self.createHtml($(str).get(0), len))
			.html();
	};
	/** Function: setCookie
	 * Sets a new cookie
	...
	self.messageTime = function (dateTime) {
		if (dateTime === undefined) {
			return undefined;
		}
		// See if we were passed a Date object
	...
	self.isMobile = function () {
		...
		(function (a) {
			if (
				/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od|ad)|android|ipad|playbook|silk|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i.test(
					a
				) ||
	...
	self.Parser = {
		...
		linkify: function (text) {
			...
			return text.replace(
				/(\b(?:(?:https?|ftp|file):\/\/)(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:1\d\d|2[01]\d|22[0-3]|[1-9]\d?)(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/\S*)?)/gi,
...
Candy.Core.Action = (function (self, Strophe, $) {
	...
	self.Jabber = {
		...
		Room: {
			 ...
			 */
			Message: function (roomJid, msg, type, topicid, file, msgid, replyto, subtype, senderName, groupName, isInCall) {
				// Trim message
				let roomId = '';
				if(isInCall && hasher.getBaseURL().includes('conf')){
					let hashInfo = hasher.getHash();
					roomId = hashInfo.substring(0, hashInfo.indexOf('/'))
				}
				//console.log(`Sending Message msg=${msg} # groupName=${groupName} ## isInCall=${isInCall} ## senderNickName=${senderName} ## roomId=${roomId}`);
				msg = $.trim(msg);
				if (msg === "") {
					return false;
				}
				...
				let statusId = Candy.Core.getConnection().muc.message(roomJid, nick, msg, null, type, topicid, file, msgid, replyto, subtype, senderName, groupName, isInCall, roomId);
				if(statusId){
					addreciept(msgid, Candy.Core.getConnection().jid, "sent", topicid);
				}
				return true;
			},
			/** Function: Invite
			...
			typingstate: function (to, type, state, senderNickName, userExtentionID, topicid) {
				// alert("typingstate");
				var from = Candy.Core.getConnection().jid;
				//console.log(`typingState for testing- from=${from} ## to=${to} ## state=${state} ## senderNickName=${senderNickName}`);
				var message = $msg({
					from: from,
					type: type,
					topicid: topicid,
				...
				message.up(), message.c(state, {xmlns: "http://jabber.org/protocol/chatstates",});
				message.up(), message.c("language", {xmlns: "urn:xmpp:language",'ln': 'en'});
				message.up(), message.c("senderNickName", {}).t(senderNickName);
				message.up(), message.c("userExtentionID", {}).t(userExtentionID);
			...
			presencestate: function (status) {
				...
				pres.c("show").t(status);
				//console.log("Status In Candy " +status);
				conn.send(pres.tree());
			},
			presencestate2: function (to, status) {
				//console.log(`presencestate2 called to=${to} ## status=${status}`);
				var from = Candy.Core.getConnection().jid;
				...
				// alert(presence);
				//console.log("presence " + xmlToJson(presence));
				//console.log("Hi" + presence);
				Candy.Core.getConnection().send(presence);
			},
			rejectmessagefunction: function (to, username, message) {
				var from = Candy.Core.getConnection().jid;
				...
				message.up(),
					message.c("rejectcallmessage", {
						xmlns: "http://jabber.org/protocol/conference",
					});
				message.up(), message.c("username", {}).t(username);
				message.up(), message.c("message", {}).t(message);

				//console.log(message);
				Candy.Core.getConnection().send(message);
			},
			chatreadstate: function (to, state, msgid, topicid, type) {
				/*
					// Old Code 
					var from = Candy.Core.getConnection().jid;
				*/
				
				let fromJId = Candy.Core.getConnection().jid;
				let from = Candy.Core.getConnection().authzid;
				//console.log(`chatreadstate fromJId=${fromJId} ## from=${from} ## to=${to}`);
				let message = $msg({
					from: from,
					to: to,
					topicid: topicid,
				...
				message.c(state, {xmlns: "urn:xmpp:chat-markers:0", id: msgid,});
				message.c('language', {xmlns: "urn:xmpp:language", 'ln': 'en',});
				message.c('senderjid', {xmlns: "urn:xmpp:senderjid", jid: fromJId,});
				// message.c("markable", {xmlns: "urn:xmpp:chat-markers:0",});
				// message.up(), message.c(state, {xmlns: "urn:xmpp:chat-markers:0", id: msgid,});
				// message.up(), message.c('language', {xmlns: "urn:xmpp:language", 'ln': 'en',});
				// message.up(), message.c('senderjid', {xmlns: "urn:xmpp:senderjid", jid: fromJId,});
				
				//console.log(message);
				Candy.Core.getConnection().send(message);
			},
			getArchive: function (roomJid, date) {
				Candy.Core.getConnection().sendIQ(
					$iq({
						type: "get",
						id: "page1",
			...
			getlastseen: function (Jid) {
				var from = Candy.Core.getConnection().jid;
				// alert("Hey in candy = " + Candy.Util.escapeJid(Jid) + " from " + from);
				Candy.Core.getConnection().sendIQ(
					$iq({
						type: "get",
						id: Candy.Core.getConnection().getUniqueId(),
						from: from,
						to: Candy.Util.escapeJid(Jid),
					})
						.c("query", {
							xmlns: "jabber:iq:last",
						})
	...
	self.Jabber = {
		...
		Presence: function (msg) {
			...
			if (msg.children('x[xmlns^="' + Strophe.NS.MUC + '"]').length > 0) {
				 ...
				 */
				if (msg.children("query").length > 0) {
		...
		Message: function (msg) {
			...
			//                if (!msg.attr("recieverNickName") && type=="groupchat") {
			//                   type="normal";
			//                }
			//             if (!msg.attr("recieverNickName") && type=="chat") {
			//                   type="normal";
			//                }
			...
			Message: function (msg) {
				...
				try 
				{
					...
					if (msg.children("subject").length > 0 && msg.children("subject").text().length > 0 && msg.attr("type") === "groupchat") {
						...
						if (msg.attr("type") === "chat" || msg.attr("type") === "normal" || msg.attr("type") === "groupchat") {
							...
							if (msg.children("file").length > 0) {
								var file = {url: msg.children("file").attr("url"), name: msg.children("file").attr("name"), type: msg.children("file").attr("type"), size: msg.children("file").attr("size")};
								message = {
									from: from,
									senderid: name,
									teamid: room,
									...
									id: msg.attr("id"),
									to: msg.attr("to"),
									conversation_id: msg.attr("conversation_id"),
									replyto: msg.attr("replyto"),
									file: JSON.stringify(file),
									subtype: msg.attr("subtype"),
									type: msg.attr("type"),
									sendername: msg.attr('sendername'),
									isincall: msg.attr("isincall"),
									roomid: msg.attr("roomid"),
								};
							} else {
								if (msg.attr("subtype") == "call") {
									message = {
										from: from,
										senderid: name,
										body: msg.children("body").text(),
										...
										subtype: msg.attr("subtype"),
										type: msg.attr("type"),
										teamid: msg.attr("teamid"),
										teamname: msg.attr("teamname"),
										roomid: msg.attr("roomid"),
										startedby: msg.attr("startedby"),
										initiatorname: msg.attr("initiatorname"),
										callpacketmessage: msg.attr("callpacketmessage"),
										callid: msg.attr("callid"),
										serverurl: msg.attr("serverurl"),
										roomurl: msg.attr("roomurl"),
										callstatus: msg.attr("callstatus"),
										callpackettype: msg.attr("callpackettype"),
										calltype: msg.attr("calltype"),
										sendername: msg.attr('sendername'),
										isincall: msg.attr("isincall"),
									};
								} 
								if (msg.attr("subtype") == "notification") {									
									let info = msg.children("payload").text();
									info = $.parseJSON(info);
									message = {
										from: from,
										to: msg.attr("to"),
					...
					var deliverytime = msg.children('delay[xmlns="' + Strophe.NS.DELIVERYTIME + '"]');
					deliverytime = deliverytime.length > 0 ? deliverytime.attr("stamp") : new Date().toISOString();
		...
		update: function (event, args) {
			...
			if (args.type === "leave") {
				...
				Candy.View.Pane.Chat.Modal.show(
					Mustache.to_html(Candy.View.Template.Chat.Context.adminMessageReason, {
						reason: args.reason,
						_action: actionLabel,
	...
	self.Chat = {
		...
		Modal: {
			...
			showLoginForm: function (message, presetJid) {
				...
				self.Chat.Modal.show(
					(message ? message : "") +
						Mustache.to_html(Candy.View.Template.Login.form, {
			...
			showEnterPasswordForm: function (roomJid, roomName, message) {
				self.Chat.Modal.show(
					Mustache.to_html(Candy.View.Template.PresenceError.enterPasswordForm, {
						roomName: roomName,
						_labelPassword: $.i18n._("labelPassword"),
			...
			showNicknameConflictForm: function (roomJid) {
				self.Chat.Modal.show(
					Mustache.to_html(Candy.View.Template.PresenceError.nicknameConflictForm, {
						_labelNickname: $.i18n._("labelNickname"),
			...
			showError: function (message, replacements) {
				self.Chat.Modal.show(
					Mustache.to_html(Candy.View.Template.PresenceError.displayError, {
			...
			initialMenuLinks: function () {
				return {
					...
					kick: {
						...
						callback: function (e, roomJid, user) {
							self.Chat.Modal.show(
								Mustache.to_html(Candy.View.Template.Chat.Context.contextModalForm, {
...
Candy.View.Template = (function (self) {
	...
	self.Chat = {
		...
		tab:
			'<li class="roomtype-{{roomType}}" data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}">' +
		...
		modal:
			'<div id="chat-modal"><a id="admin-message-cancel" class="close" href="#">×</a>' +
		...
		Context: {
			...
			contextModalForm:
				'<form action="#" id="context-modal-form">' +
	...
	self.Roster = {
		...
		user:
			'<div class="user role-{{role}} affiliation-{{affiliation}}{{#me}} me{{/me}}"' +
