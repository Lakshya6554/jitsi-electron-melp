...
function core_sha1(a, b) {
	...
	for (c = 0; c < a.length; c += 16) {
		for (f = l, g = m, h = n, i = o, j = p, d = 0; 80 > d; d++)
			16 > d ? (k[d] = a[c + d]) : (k[d] = rol(k[d - 3] ^ k[d - 8] ^ k[d - 14] ^ k[d - 16], 1)),
...
Function.prototype.bind ||
	...
	(function (a) {
		...
		(f = {
			...
			XHTML: {
				...
				validTag: function (a) {
					for (var b = 0; b < f.XHTML.tags.length; b++) if (a == f.XHTML.tags[b]) return !0;
					return !1;
				},
				validAttribute: function (a, b) {
					if ("undefined" != typeof f.XHTML.attributes[a] && f.XHTML.attributes[a].length > 0)
						for (var c = 0; c < f.XHTML.attributes[a].length; c++) if (b == f.XHTML.attributes[a][c]) return !0;
					return !1;
				},
				validCSS: function (a) {
					for (var b = 0; b < f.XHTML.css.length; b++) if (a == f.XHTML.css[b]) return !0;
					return !1;
				},
			},
			Status: {
			...
			_makeGenerator: function () {
				var a;
				return (
					void 0 === document.implementation.createDocument || (document.implementation.createDocument && document.documentMode && document.documentMode < 10)
						? ((a = this._getIEXmlDom()), a.appendChild(a.createElement("strophe")))
						: (a = document.implementation.createDocument("jabber:client", "strophe", null)),
					a
				);
			},
					...
					if (arguments[b])
						if ("string" == typeof arguments[b] || "number" == typeof arguments[b]) e.appendChild(f.xmlTextNode(arguments[b]));
							...
							for (c = 0; c < arguments[b].length; c++)
								"object" == typeof arguments[b][c] && "function" == typeof arguments[b][c].sort && e.setAttribute(arguments[b][c][0], arguments[b][c][1]);
						else if ("object" == typeof arguments[b]) for (d in arguments[b]) arguments[b].hasOwnProperty(d) && e.setAttribute(d, arguments[b][d]);
				return e;
			},
			...
			createHtml: function (a) {
				...
				if (a.nodeType == f.ElementType.NORMAL)
									...
									if (("style" == g && "object" == typeof h && "undefined" != typeof h.cssText && (h = h.cssText), "style" == g)) {
										for (i = [], j = h.split(";"), d = 0; d < j.length; d++)
											(k = j[d].split(":")),
												...
												f.XHTML.validCSS(l) && ((m = k[1].replace(/^\s*/, "").replace(/\s*$/, "")), i.push(l + ": " + m));
					...
					else for (c = f.xmlGenerator().createDocumentFragment(), b = 0; b < a.childNodes.length; b++) c.appendChild(f.createHtml(a.childNodes[b]));
			...
			serialize: function (a) {
				...
				var c,
					d,
					e = a.nodeName;
				for (a.getAttribute("_realname") && (e = a.getAttribute("_realname")), b = "<" + e, c = 0; c < a.attributes.length; c++)
					"_realname" != a.attributes[c].nodeName &&
						(b +=
							" " +
							a.attributes[c].nodeName.toLowerCase() +
							"='" +
							a.attributes[c].value.replace(/&/g, "&amp;").replace(/\'/g, "&apos;").replace(/>/g, "&gt;").replace(/</g, "&lt;") +
							"'");
			...
			(f.Handler = function (a, b, c, d, e, g, h) {
				(this.handler = a),
					...
					this.options.matchBare ? (this.from = g ? f.getBareJidFromJid(g) : null) : (this.from = g),
					(this.user = !0);
			}),
			(f.Handler.prototype = {
				isMatch: function (a) {
				...
				run: function (a) {
					...
					try {
						...
						throw (
							(c.sourceURL
								? f.fatal("error: " + this.handler + " " + c.sourceURL + ":" + c.line + " - " + c.name + ": " + c.message)
								: c.fileName
								? ("undefined" != typeof console && (console.trace(), console.error(this.handler, " - error - ", c, c.message)),
								  f.fatal("error: " + this.handler + " " + c.fileName + ":" + c.lineNumber + " - " + c.name + ": " + c.message))
								: f.fatal("error: " + c.message + "\n" + c.stack),
							c)
						);
					}
					return b;
			...
			(f.Connection = function (a, b) {
				...
				0 === a.indexOf("ws:") || 0 === a.indexOf("wss:") || 0 === c.indexOf("ws") ? (this._proto = new f.Websocket(this)) : (this._proto = new f.Bosh(this)),
					(this.jid = ""),
					(this.domain = null),
					(this.features = null),
					(this._sasl_data = {}),
			...
			(f.Connection.prototype = {
				...
				connect: function (a, b, c, d, e, g) {
					Strophe.info("connect first method called a=" + a + " ## c=" + c + " ## d=" + d + " ## e=" + e + " ## g=" + g);
					(this.jid = a),
					...
					Strophe.info("connect first method called authcid=" + this.authcid + " ## authzid=" + this.authzid);
				},
				...
				send: function (a) {
					Strophe.info("send Method called Sending Packet = > " + JSON.stringify(a)); //on status called
					if (null !== a) {
						if ("function" == typeof a.sort) for (var b = 0; b < a.length; b++) this._queueData(a[b]);
				...
				sendIQ: function (a, b, c, d) {
					Strophe.info("sendIQ method called a=" + a + " ## b=" + b + " ## c=" + c + " ## d=" + d);
					var e = null,
						f = this;
					"function" == typeof a.tree && (a = a.tree());
					...
					Strophe.info("sendIQ other case a=" + a);
					return (
						d &&
				...
				_changeConnectStatus: function (a, b) {
					f.info("_changeConnectStatus called a=" + JSON.stringify(a) + " ## b=" + JSON.stringify(b));
					for (var c in f._connectionPlugins)
						if (f._connectionPlugins.hasOwnProperty(c)) {
							var d = this[c];
							f.info("_changeConnectStatus called d=" + d);
							if (d.statusChanged)
								try {
									f.info("_changeConnectStatus called d.statusChanged=" + d.statusChanged);
									d.statusChanged(a, b);
								} catch (e) {
					...
					if (this.connect_callback)
						try {
							f.info("_changeConnectStatus  connect_callback called connect_callback=" + this.connect_callback);
							this.connect_callback(a, b);
						} catch (g) {
							f.error("User connection callback caused an exception: " + g);
						}
					f.info("_changeConnectStatus end ");
				},
				_doDisconnect: function () {
					...
					null !== this._disconnectTimeout && (this.deleteTimedHandler(this._disconnectTimeout), (this._disconnectTimeout = null)),
						...
						(this.connected = !1);
					f.info("_doDisconnect was end ");
				},
				_dataRecv: function (a, b) {
					f.info("_dataRecv called");
					var c = this._proto._reqToData(a);
					f.info("_dataRecv called c=" + JSON.stringify(c));
					if (null !== c) {
						this.xmlInput !== f.Connection.prototype.xmlInput &&
							(c.nodeName === this._proto.strip && c.childNodes.length ? this.xmlInput(c.childNodes[0]) : this.xmlInput(c)),
							this.rawInput !== f.Connection.prototype.rawInput && (b ? this.rawInput(b) : this.rawInput(f.serialize(c)));
						...
						if (null !== i && "terminate" == i) {
							...
							return (
								...
								null !== g
									? ("remote-stream-error" == g && h.length > 0 && (g = "conflict"), this._changeConnectStatus(f.Status.CONNFAIL, g))
				...
				_connect_cb: function (a, b, c) {
					f.info("_connect_cb was called a=" + JSON.stringify(a) + " ## b=" + b + " ## c=" + c), (this.connected = !0);
					var d = this._proto._reqToData(a);
					f.info("other para d=" + d);
					if (d) {
						this.xmlInput !== f.Connection.prototype.xmlInput &&
							(d.nodeName === this._proto.strip && d.childNodes.length ? this.xmlInput(d.childNodes[0]) : this.xmlInput(d)),
							this.rawInput !== f.Connection.prototype.rawInput && (c ? this.rawInput(c) : this.rawInput(f.serialize(d)));
						f.info("other-2 para d=" + JSON.stringify(d));
						var e = this._proto._connect_cb(d);
						f.info("other para e=" + JSON.stringify(e) + " ## f.status.ConnFail=" + f.Status.CONNFAIL);
						if (e !== f.Status.CONNFAIL) {
							...
							if (j.length > 0)
								for (h = 0; h < j.length; h++)
									(i = f.getText(j[h])), f.info("other para mechanisms[i]=" + this.mechanisms[i]), this.mechanisms[i] && k.push(this.mechanisms[i]);
							return (
								(this._authentication.legacy_auth = d.getElementsByTagName("auth").length > 0),
								(l = this._authentication.legacy_auth || k.length > 0)
									? void (this.do_authentication !== !1 && this.authenticate(k))
									: void this._proto._no_auth_received(b)
							);
						}
					}
				},
				authenticate: function (a) {
					f.info("authenticate method called a=" + JSON.stringify(a) + " ## Other a=" + a);
					var c;
					...
					i ||
						(null === f.getNodeFromJid(this.jid)
							? (this._changeConnectStatus(f.Status.CONNFAIL, "x-strophe-bad-non-anon-jid"), this.disconnect("x-strophe-bad-non-anon-jid"))
				...
				_auth1_cb: function (a) {
					var b = d({
						...
						.t(this.pass);
					return (
						f.getResourceFromJid(this.jid) || (this.jid = f.getBareJidFromJid(this.jid) + "/strophe"),
				...
				_sasl_success_cb: function (a) {
					if (this._sasl_data["server-signature"]) {
						var b,
							...
							d = /([a-z]+)=([^,]+)(,|$)/,
							e = c.match(d);
						if (("v" == e[1] && (b = e[2]), b != this._sasl_data["server-signature"]))
							return (
					...
					return (
						f.info("SASL authentication succeeded-1."),
						...
						? ((e = g[0].getElementsByTagName("jid")),
						  void (
								...
								this.do_session
									...
									: ((this.authenticated = !0), this._changeConnectStatus(f.Status.CONNECTED, null)))
						  ))
						: (f.info("SASL binding failed."), this._changeConnectStatus(f.Status.AUTHFAIL, null), !1);
				},
				...
				_auth2_cb: function (a) {
					return (
						"result" == a.getAttribute("type")
							? ((this.authenticated = !0), this._changeConnectStatus(f.Status.CONNECTED, null))
							: "error" == a.getAttribute("type") && (this._changeConnectStatus(f.Status.AUTHFAIL, null), this.disconnect("authentication failed")),
						!1
					);
				},
				...
				_onIdle: function () {
					...
					for (d = [], a = 0; a < this.timedHandlers.length; a++)
						(b = this.timedHandlers[a]), (this.authenticated || !b.user) && ((c = b.lastCalled + b.period), 0 >= c - e ? b.run() && d.push(b) : d.push(b));
			...
			(f.SASLMD5.prototype.onChallenge = function (a, b, c) {
				for (var d, e = /([a-z]+)=("[^"]+"|[^,"]+)(?:,|$)/, f = c || MD5.hexdigest("" + 1234567890 * Math.random()), g = "", h = null, i = "", j = ""; b.match(e); )
					switch (((d = b.match(e)), (b = b.replace(d[0], "")), (d[2] = d[2].replace(/^"(.+)"$/, "$1")), d[1])) {
						case "realm":
							g = d[2];
							break;
						case "nonce":
				...
				return (
					(n += "charset=utf-8,"),
					(n += "username=" + this._quote(a.authcid) + ","),
					(n += "realm=" + this._quote(g) + ","),
					(n += "nonce=" + this._quote(i) + ","),
					(n += "nc=00000001,"),
					(n += "cnonce=" + this._quote(f) + ","),
					(n += "digest-uri=" + this._quote(k) + ","),
					(n += "response=" + MD5.hexdigest(MD5.hexdigest(l) + ":" + i + ":00000001:" + f + ":auth:" + MD5.hexdigest(m)) + ","),
					(n += "qop=auth"),
	...
	(Strophe.Request.prototype = {
		getResponse: function () {
			...
			if (this.xhr.responseXML && this.xhr.responseXML.documentElement) {
				if (((a = this.xhr.responseXML.documentElement), "parsererror" == a.tagName))
					throw (
		...
		_newXHR: function () {
			...
			return (
				window.XMLHttpRequest
					? ((a = new XMLHttpRequest()), a.overrideMimeType && a.overrideMimeType("text/xml"))
	...
	(Strophe.Bosh = function (a) {
		(this._conn = a), (this.rid = Math.floor(4294967295 * Math.random())), (this.sid = null), (this.hold = 1), (this.wait = 15000), (this.window = 5), (this._requests = []);
		//this._conn = a, this.rid = Math.floor(4294967295 * Math.random()), this.sid = null, this.hold = 3, this.wait = 120, this.window = 5, this._requests = []
	}),
	(Strophe.Bosh.prototype = {
		...
		_buildBody: function () {
			...
			Strophe.info("_buildBody Called-  SID  " + this.sid + " ## a=" + a);
			return (
				null !== this.sid &&
					a.attrs({
						sid: this.sid,
		...
		_connect: function (a, b, c) {
			Strophe.info("_connect called " + sessionId);
			...
			Strophe.info(
				"_connect called e=" +
					...
					" ## sid=" +
					d.tree().getAttribute("sid")
			);

			this._requests.push(new Strophe.Request(d.tree(), this._onRequestStateChange.bind(this, e.bind(this._conn)), d.tree().getAttribute("rid"))),
		...
		_connect_cb: function (a) {
			Strophe.info("_connect_cb called " + sessionId + " ## SID=" + a.getAttribute("sid"));
			var b,
				c,
			...
			if (null !== d && "terminate" == d)
				return (
					...
					(c = a.getElementsByTagName("conflict")),
					null !== b
						? ("remote-stream-error" == b && c.length > 0 && (b = "conflict"), this._conn._changeConnectStatus(Strophe.Status.CONNFAIL, b))
						: this._conn._changeConnectStatus(Strophe.Status.CONNFAIL, "unknown"),
					this._conn._doDisconnect(),
					Strophe.Status.CONNFAIL
				);
			this.sid || (this.sid = a.getAttribute("sid"));
			//this.sid || (this.sid = NSID);
		...
		_doDisconnect: function () {
			Strophe.info("_doDisconnect called ");
		...
		_hitError: function (a) {
			Strophe.info("_hitError called a=" + JSON.stringify(a));
			this.errors++, Strophe.warn("request errored, status: " + a + ", number of errors: " + this.errors), this.errors > 4 && this._onDisconnectTimeout();
			Strophe.info("_hitError End error=" + this.errors);
		},
		_no_auth_received: function (a) {
			Strophe.info("_no_auth_received Called a=" + JSON.stringify(a) + " ## conn=" + JSON.stringify(this._conn));
			a = a ? a.bind(this._conn) : this._conn._connect_cb.bind(this._conn);
			var b = this._buildBody();
			Strophe.info("_no_auth_received check b=" + JSON.stringify(b));
		...
		_onDisconnectTimeout: function () {
			Strophe.info("_onDisconnectTimeout called request=" + JSON.stringify(this._requests));
			for (var a; this._requests.length > 0; ) (a = this._requests.pop()), (a.abort = !0), a.xhr.abort(), (a.xhr.onreadystatechange = function () {});

			Strophe.info("_onDisconnectTimeout end ");
		},
		_onIdle: function () {
			Strophe.info("_onIdle called " + this._conn._data);
			...
			if (
				(this._conn.authenticated &&
					...
					(Strophe.info("no requests during idle cycle, sending blank request"), a.push(null)),
				this._requests.length < 2 && a.length > 0 && !this._conn.paused)
			) {
				for (var b = this._buildBody(), c = 0; c < a.length; c++)
					null !== a[c] &&
						("restart" === a[c]
							? b.attrs({
									to: this._conn.domain,
				...
				delete this._conn._data,
					(this._conn._data = []),
					this._requests.push(new Strophe.Request(b.tree(), this._onRequestStateChange.bind(this, this._conn._dataRecv.bind(this._conn)), b.tree().getAttribute("rid"))),
					this._processRequest(this._requests.length - 1);
			}
			if (this._requests.length > 0) {
				var d = this._requests[0].age();
				null !== this._requests[0].dead && this._requests[0].timeDead() > Math.floor(Strophe.SECONDARY_TIMEOUT * this.wait) && this._throttledRequestHandler(),
					d > Math.floor(Strophe.TIMEOUT * this.wait) &&
						(Strophe.warn("Request " + this._requests[0].id + " timed out, over " + Math.floor(Strophe.TIMEOUT * this.wait) + " seconds since last activity"),
						this._throttledRequestHandler());
			}
		},
		_onRequestStateChange: function (a, b) {
			Strophe.info("_onRequestStateChange = " + JSON.stringify(a) + " ## b=" + JSON.stringify(b) + " ## xhr=" + JSON.stringify(b.xhr));
			if ((Strophe.debug("request id " + b.id + "." + b.sends + " state changed to " + b.xhr.readyState), b.abort)) return void (b.abort = !1);

			var c;
			Strophe.info("readyState=" + b.xhr.readyState + " ## status==" + b.xhr.status);
			if (4 == b.xhr.readyState) {
				c = 0;
				try {
					c = b.xhr.status;
				} catch (d) {
					Strophe.info("_onRequestStateChange this is d= " + d);
				}
				...
				Strophe.info("_onRequestStateChange == e=" + e + " ## f=" + f + " ## c=" + c);
				((c > 0 && 500 > c) || b.sends > 5) && (this._removeRequest(b), Strophe.debug("request id " + b.id + " should now be removed")),
					200 == c
						? ((f || (e && this._requests.length > 0 && this._requests[0].age() > Math.floor(Strophe.SECONDARY_TIMEOUT * this.wait))) && this._restartRequest(0),
						  Strophe.debug("request id " + b.id + "." + b.sends + " got 200"),
						  a(b),
						  (this.errors = 0))
						: (Strophe.error("request id " + b.id + "." + b.sends + " error " + c + " happened"),
						  (0 === c || (c >= 400 && 600 > c) || c >= 12e3) &&
								(this._hitError(c), c >= 400 && 500 > c && (this._conn._changeConnectStatus(Strophe.Status.DISCONNECTING, null), this._conn._doDisconnect()))),
					(c > 0 && 500 > c) || b.sends > 5 || this._throttledRequestHandler();
			}
		},
		_processRequest: function (a) {
			Strophe.info("_processRequest == a=" + a + " ## stringify=" + JSON.stringify(a));
			var b = this,
				c = this._requests[a],
				d = -1;
			try {
			...
			if (
				((g || h || i) &&
					(h && Strophe.error("Request " + this._requests[a].id + " timed out (secondary), restarting"),
					(c.abort = !0),
					c.xhr.abort(),
					(c.xhr.onreadystatechange = function () {}),
					(this._requests[a] = new Strophe.Request(c.xmlData, c.origFunc, c.rid, c.sends)),
					(c = this._requests[a])),
				0 === c.xhr.readyState)
			) {
				...
				try {
					...
					return (
						...
						//console.log(`XHR open failed. connected >> ${Strophe.Status.CONNFAIL} ## ${isResetConnCall}`),
						this._conn.connected || this._conn._changeConnectStatus(Strophe.Status.CONNFAIL, "bad-service"),
				...
				var k = function () {
					...
					var session = "><sessionId>" + sessionId + "</sessionId";

					Strophe.info("c.data-1 = " + c.data);
					c.data = c.data.replace("sessionid", "sessionId");
					Strophe.info("c.data-2 = " + c.data);
					//Strophe.info('IS_ONLINE=='+IS_ONLINE);

					//if(IS_ONLINE){
					if (navigator.onLine) {
						try {
							var resXhr = c.xhr.send(c.data);
							//Strophe.info("resXhr " +resXhr);
						} catch (exception) {
							//console.log(`resXhr exception=${exception} ## ${JSON.stringify(exception)}`)
							console.log(exception);
						}
					}
				};
				Strophe.info("c.sends = " + c.sends);
				if (c.sends > 1) {
					var l = 1e3 * Math.min(Math.floor(Strophe.TIMEOUT * this.wait), Math.pow(c.sends, 3));
					setTimeout(k, l);
				} else k();
				c.sends++,
					//console.log(`xmlData >>`, c.xmlData, ' ## ', c.data, ' ## isResetConnCall=', isResetConnCall),
					this._conn.xmlOutput !== Strophe.Connection.prototype.xmlOutput &&
						(c.xmlData.nodeName === this.strip && c.xmlData.childNodes.length ? this._conn.xmlOutput(c.xmlData.childNodes[0]) : this._conn.xmlOutput(c.xmlData)),
					this._conn.rawOutput !== Strophe.Connection.prototype.rawOutput && this._conn.rawOutput(c.data);
			} else Strophe.debug("_processRequest: " + (0 === a ? "first" : "second") + " request has readyState of " + c.xhr.readyState);
		},
		_removeRequest: function (a) {
			Strophe.info("removing request");
			var b;
		...
		_restartRequest: function (a) {
			Strophe.info(
				"_restartRequest Called a=" +
					...
					this._requests[a] +
					" ## _request.stringify=" +
					JSON.stringify(this._requests[a])
			);
		...
		_send: function () {
			//    alert("om");
			Strophe.info("_send method called _conn=" + this._conn);
		...
		_throttledRequestHandler: function () {
			Strophe.info("_throttledRequestHandler was called " + JSON.stringify(this._requests));
			this._requests
				...
				this._requests &&
					...
					this._requests.length > 1 && Math.abs(this._requests[0].rid - this._requests[1].rid) < this.window && this._processRequest(1));
			Strophe.info("_throttledRequestHandler was end");
		},
	}),
	(Strophe.Websocket = function (a) {
		...
		if (0 !== b.indexOf("ws:") && 0 !== b.indexOf("wss:")) {
			var c = "";
			(c += "ws" === a.options.protocol && "https:" !== window.location.protocol ? "ws" : "wss"),
				(c += "://" + window.location.host),
				(c += 0 !== b.indexOf("/") ? window.location.pathname + b : b),
				(a.service = c);
		}
	}),
	(Strophe.Websocket.prototype = {
		...
		_handleStreamStart: function (a) {
			...
			return (
				"string" != typeof e ? (b = "Missing version in stream:stream") : "1.0" !== e && (b = "Wrong version in stream:stream: " + e),
				b ? (this._conn._changeConnectStatus(Strophe.Status.CONNFAIL, b), this._conn._doDisconnect(), !1) : !0
			);
		},
		_connect_cb_wrapper: function (a) {
			if (0 === a.data.indexOf("<stream:stream ") || 0 === a.data.indexOf("<?xml")) {
				...
				this._conn.xmlInput(c),
					...
					this._handleStreamStart(c) && (this._connect_cb(c), (this.streamStart = a.data.replace(/^<stream:(.*)\/>$/, "<stream:$1>")));
			} else {
		...
		_onIdle: function () {
			...
			if (a.length > 0 && !this._conn.paused) {
				for (var b = 0; b < a.length; b++)
					if (null !== a[b]) {
						var c, d;
						"restart" === a[b] ? ((c = this._buildStream()), (d = this._removeClosingTag(c)), (c = c.tree())) : ((c = a[b]), (d = Strophe.serialize(c))),
		...
		_onMessage: function (a) {
			...
			if (0 === a.data.search("<stream:stream ")) {
				if (
					((c = a.data.replace(/<stream:stream (.*[^\/])>/, "<stream:stream $1/>")),
					(b = new DOMParser().parseFromString(c, "text/xml").documentElement),
			...
			if (!this._check_streamerror(b, Strophe.Status.ERROR))
				return this._conn.disconnecting && "presence" === b.firstChild.nodeName && "unavailable" === b.firstChild.getAttribute("type")
					? (this._conn.xmlInput(b), void this._conn.rawInput(Strophe.serialize(b)))
	...
	function () {
		...
		Strophe.addConnectionPlugin("muc", {
			...
			join: function (a, b, d, e, f, g, h) {
				var i, j;
				return (
					(j = this.test_append_nick(a, b)),
					(i = $pres({
					...
					null != h && (i = i.c("history", h).up),
					null != g && i.cnode(Strophe.xmlElement("password", [], g)),
					"undefined" != typeof extended_presence && null !== extended_presence && i.up.cnode(extended_presence),
					null == this._muc_handler &&
						(this._muc_handler = this._connection.addHandler(
							(function (b) {
								return function (c) {
									var d, e, f, g, h, i, j, k, l, m;
									if (((d = c.getAttribute("from")), !d)) return !0;
									if (((h = d.split("/")[0]), !b.rooms[h])) return !0;
									if (((a = b.rooms[h]), (f = {}), "message" === c.nodeName)) f = a._message_handlers;
									else if ("presence" === c.nodeName && ((k = c.getElementsByTagName("x")), k.length > 0))
										for (l = 0, m = k.length; m > l; l++)
											if (((i = k[l]), (j = i.getAttribute("xmlns")), j && j.match(Strophe.NS.MUC))) {
												f = a._presence_handlers;
												break;
											}
									for (g in f) (e = f[g]), e(c, a) || delete f[g];
									return !0;
								};
							})(this)
						)),
					this.rooms.hasOwnProperty(a) || ((this.rooms[a] = new c(this, a, b, g)), this.roomNames.push(a)),
					e && this.rooms[a].addHandler("presence", e),
					d && this.rooms[a].addHandler("message", d),
					f && this.rooms[a].addHandler("roster", f),
					this._connection.send(i)
				);
			},
			leave: function (a, b, c, d) {
				var e, f, g, h;
				return (
					(e = this.roomNames.indexOf(a)),
					delete this.rooms[a],
					e >= 0 && (this.roomNames.splice(e, 1), 0 === this.roomNames.length && (this._connection.deleteHandler(this._muc_handler), (this._muc_handler = null))),
					(h = this.test_append_nick(a, b)),
					(g = this._connection.getUniqueId()),
					(f = $pres({
						type: "unavailable",
						id: g,
						from: this._connection.jid,
					...
					null != d && f.c("status", d),
					null != c && this._connection.addHandler(c, null, "presence", null, g),
					this._connection.send(f),
					g
				);
			},
			message: function (a, b, c, d, e, topicid, file, msgid, replyto, subtype, senderName, groupName, isInCall, roomId) {
				//alert(b + c + d + e);
				if (subtype == "call") {
					c = ""; //body uda di
					//   console.log("here ? " + JSON.stringify(callobj_jiti_message_packet));
					var newToExtn = callobj_jiti_message_packet["newToExtn"];
					var newFromExtn = this._connection.jid; //callobj_jiti_message_packet['newFromExtn'];
					var newType = callobj_jiti_message_packet["newType"];
					var newId = callobj_jiti_message_packet["newId"];
					var newsubtype = callobj_jiti_message_packet["newsubtype"];
					var conversation_id = callobj_jiti_message_packet["conversation_id"];
					var newteamid = callobj_jiti_message_packet["newteamid"];
					var newteamname = callobj_jiti_message_packet["newteamname"];
					var roomid = callobj_jiti_message_packet["roomid"];
					var newstartedby = callobj_jiti_message_packet["newstartedby"];
					var newinitiatorname = callobj_jiti_message_packet["newinitiatorname"];
					var newcallid = callobj_jiti_message_packet["newcallid"];
					var newserverurl = callobj_jiti_message_packet["newserverurl"];
					var newroomurl = callobj_jiti_message_packet["newroomurl"];
					var newcallstatus = callobj_jiti_message_packet["newcallstatus"];
					var newcallpackettype = callobj_jiti_message_packet["newcallpackettype"];
					var newcalltype = callobj_jiti_message_packet["newcalltype"];

					file = null;
					var f, g, h, i;
					return (
						(i = this.test_append_nick(a, b)),
						(e = e || (null != b ? "chat" : "groupchat")),
						(g = this._connection.getUniqueId()),
						(f = $msg({
							to: newToExtn,
							from: newFromExtn,
							...
							initiatorname: newinitiatorname,
							callid: newcallid,
							serverurl: newserverurl,
							roomurl: newroomurl,
							callstatus: newcallstatus,
							callpackettype: newcallpackettype,
						...
						null != d &&
							(f
								...
								.h(d),
							0 === f.node.childNodes.length ? ((h = f.node.parentNode), f.up().up(), f.node.removeChild(h)) : f.up().up()),
						null != file && (f.c("file", {name: name, type: type, size: sizeVal, url: url}), f.up()),
						f.c("markable", {
					...
					if (file) {
						//                       alert("Hello " + JSON.stringify(file));
						var type = file["type"];
						var name = file["name"];						
						var url = file["url"];
						// var sizeValue = file["size"];
						// sizeValue = parseFloat(sizeValue) / 1024;
						// var sizeVal = "" + sizeValue + " KB";
						//var sizeVal = bytesToSize(file["size"]);
						var sizeVal= $.isNumeric(file.size) ? bytesToSize(file.size) : file.size;
					}
					//                    alert(type);
					var f, g, h, i;
					return (
			...
			groupchat: function (a, b, c) {
				return this.message(a, null, b, c);
			},
			invite: function (a, b, c) {
				var d, e;
				return (
					(e = this._connection.getUniqueId()),
					(d = $msg({
						from: this._connection.jid,
						to: a,
						id: e,
			...
			cancelConfigure: function (a) {
				...
				return (
					...
					(c = b.tree()),
					this._connection.sendIQ(c)
				);
			},
			saveConfiguration: function (a, b, c, d) {
				var e, f, g, h, i;
				if (
					((f = $iq({
						to: a,
					...
					"undefined" != typeof Form && b instanceof Form)
				)
					(b.type = "submit"), f.cnode(b.toXML());
				else
					for (
						f.c("x", {
							xmlns: "jabber:x:data",
						...
						(e = b[h]), f.cnode(e).up();
				return (g = f.tree()), this._connection.sendIQ(g, c, d);
			},
			createInstantRoom: function (a, b, c) {
				var d;
				return (
					(d = $iq({
						to: a,
					...
					this._connection.sendIQ(d.tree(), b, c)
				);
			},
			setTopic: function (a, b) {
				var c;
				return (
					(c = $msg({
						to: a,
						...
						.t(b)),
					this._connection.send(c.tree())
				);
			},
			_modifyPrivilege: function (a, b, c, d, e) {
				var f;
				return (
					(f = $iq({
						to: a,
						...
						.cnode(b.node)),
					null != c && f.c("reason", c),
					this._connection.sendIQ(f.tree(), d, e)
				);
			},
			modifyRole: function (a, b, c, d, e, f) {
				var g;
				return (
					(g = $build("item", {
						nick: b,
					...
					this._modifyPrivilege(a, g, d, e, f)
				);
			},
			kick: function (a, b, c, d, e) {
				return this.modifyRole(a, b, "none", c, d, e);
			},
			voice: function (a, b, c, d, e) {
				return this.modifyRole(a, b, "participant", c, d, e);
			},
			mute: function (a, b, c, d, e) {
				return this.modifyRole(a, b, "visitor", c, d, e);
			},
			op: function (a, b, c, d, e) {
				return this.modifyRole(a, b, "moderator", c, d, e);
			},
			deop: function (a, b, c, d, e) {
				return this.modifyRole(a, b, "participant", c, d, e);
			},
			modifyAffiliation: function (a, b, c, d, e, f) {
				var g;
				return (
					(g = $build("item", {
						jid: b,
			...
			ban: function (a, b, c, d, e) {
				return this.modifyAffiliation(a, b, "outcast", c, d, e);
			},
			member: function (a, b, c, d, e) {
				return this.modifyAffiliation(a, b, "member", c, d, e);
			},
			revoke: function (a, b, c, d, e) {
				return this.modifyAffiliation(a, b, "none", c, d, e);
			},
			owner: function (a, b, c, d, e) {
				return this.modifyAffiliation(a, b, "owner", c, d, e);
			},
			admin: function (a, b, c, d, e) {
				return this.modifyAffiliation(a, b, "admin", c, d, e);
			},
			changeNick: function (a, b) {
				var c, d;
				return (
					(d = this.test_append_nick(a, b)),
					(c = $pres({
						from: this._connection.jid,
						to: d,
			...
			setStatus: function (a, b, c, d) {
				var e, f;
				return (
					(f = this.test_append_nick(a, b)),
					(e = $pres({
						from: this._connection.jid,
						to: f,
					})),
					null != c && e.c("show", c).up(),
					null != d && e.c("status", d),
					this._connection.send(e.tree())
				);
			},
			listRooms: function (a, b, c) {
				var d;
				return (
					(d = $iq({
						to: a,
					...
					this._connection.sendIQ(d, b, c)
				);
			},
			test_append_nick: function (a, b) {
				var c, d;
				return (d = Strophe.escapeNode(Strophe.getNodeFromJid(a))), (c = Strophe.getDomainFromJid(a)), d + "@" + c + (null != b ? "/" + b : "");
			},
		}),
			(c = (function () {
				function b(a, b, c, e) {
					(this.client = a),
						...
						(this.nick = c),
						(this.password = e),
						(this._roomRosterHandler = d(this._roomRosterHandler, this)),
						(this._addOccupant = d(this._addOccupant, this)),
						(this.roster = {}),
						(this._message_handlers = {}),
						(this._presence_handlers = {}),
						(this._roster_handlers = {}),
						(this._handler_ids = 0),
						a.muc && (this.client = a.muc),
						(this.name = Strophe.getBareJidFromJid(b)),
						this.addHandler("presence", this._roomRosterHandler);
				}
				return (
					(b.prototype.join = function (a, b, c) {
						return this.client.join(this.name, this.nick, a, b, c, this.password);
					}),
					(b.prototype.leave = function (a, b) {
						return this.client.leave(this.name, this.nick, a, b), delete this.client.rooms[this.name];
					}),
					(b.prototype.message = function (a, b, c, d) {
						return this.client.message(this.name, a, b, c, d);
					}),
					(b.prototype.groupchat = function (a, b) {
						return this.client.groupchat(this.name, a, b);
					}),
					(b.prototype.invite = function (a, b) {
						return this.client.invite(this.name, a, b);
					}),
					(b.prototype.directInvite = function (a, b) {
						return this.client.directInvite(this.name, a, b, this.password);
					}),
					(b.prototype.configure = function (a) {
						return this.client.configure(this.name, a);
					}),
					(b.prototype.cancelConfigure = function () {
						return this.client.cancelConfigure(this.name);
					}),
					(b.prototype.saveConfiguration = function (a) {
						return this.client.saveConfiguration(this.name, a);
					}),
					(b.prototype.queryOccupants = function (a, b) {
						return this.client.queryOccupants(this.name, a, b);
					}),
					(b.prototype.setTopic = function (a) {
						return this.client.setTopic(this.name, a);
					}),
					(b.prototype.modifyRole = function (a, b, c, d, e) {
						return this.client.modifyRole(this.name, a, b, c, d, e);
					}),
					(b.prototype.kick = function (a, b, c, d) {
						return this.client.kick(this.name, a, b, c, d);
					}),
					(b.prototype.voice = function (a, b, c, d) {
						return this.client.voice(this.name, a, b, c, d);
					}),
					...
					(b.prototype.modifyAffiliation = function (a, b, c, d, e) {
						return this.client.modifyAffiliation(this.name, a, b, c, d, e);
					}),
					(b.prototype.ban = function (a, b, c, d) {
					...
					(b.prototype.revoke = function (a, b, c, d) {
						return this.client.revoke(this.name, a, b, c, d);
					}),
					(b.prototype.owner = function (a, b, c, d) {
						return this.client.owner(this.name, a, b, c, d);
					}),
					(b.prototype.admin = function (a, b, c, d) {
						return this.client.admin(this.name, a, b, c, d);
					}),
					(b.prototype.changeNick = function (a) {
						return (this.nick = a), this.client.changeNick(this.name, a);
					}),
					(b.prototype.setStatus = function (a, b) {
						return this.client.setStatus(this.name, this.nick, a, b);
					}),
					(b.prototype.addHandler = function (a, b) {
						var c;
						switch (((c = this._handler_ids++), a)) {
							case "presence":
								this._presence_handlers[c] = b;
								break;
							case "message":
								this._message_handlers[c] = b;
								break;
							case "roster":
								this._roster_handlers[c] = b;
								break;
							default:
					...
					(b.prototype.removeHandler = function (a) {
						return delete this._presence_handlers[a], delete this._message_handlers[a], delete this._roster_handlers[a];
					}),
					(b.prototype._addOccupant = function (b) {
						var c;
						return (c = new a(b, this)), (this.roster[c.nick] = c), c;
					}),
					(b.prototype._roomRosterHandler = function (a) {
						var c, d, e, f, g, h;
						switch (((c = b._parsePresence(a)), (g = c.nick), (f = c.newnick || null), c.type)) {
							case "error":
								return;
							case "unavailable":
								f &&
									((c.nick = f),
									this.roster[g] && this.roster[f] && (this.roster[g].update(this.roster[f]), (this.roster[f] = this.roster[g])),
									this.roster[g] && !this.roster[f] && (this.roster[f] = this.roster[g].update(c))),
									delete this.roster[g];
								break;
							default:
								this.roster[g] ? this.roster[g].update(c) : this._addOccupant(c);
						}
						h = this._roster_handlers;
						for (e in h) (d = h[e]), d(this.roster, this) || delete this._roster_handlers[e];
						return !0;
					}),
					(b._parsePresence = function (a) {
						var b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q;
						for (
							e = {},
								b = a.attributes,
								e.nick = Strophe.getResourceFromJid(b.from.textContent),
								e.type = (null != (j = b.type) ? j.textContent : void 0) || null,
								e.states = [],
								k = a.childNodes,
								f = 0,
								h = k.length;
							...
							switch (((c = k[f]), c.nodeName)) {
								case "status":
									e.status = c.textContent || null;
									break;
								case "show":
									e.show = c.textContent || null;
									break;
								case "x":
									if (((b = c.attributes), (null != (l = b.xmlns) ? l.textContent : void 0) === Strophe.NS.MUC_USER))
										for (m = c.childNodes, g = 0, i = m.length; i > g; g++)
											switch (((d = m[g]), d.nodeName)) {
												case "item":
													(b = d.attributes),
														(e.affiliation = (null != (n = b.affiliation) ? n.textContent : void 0) || null),
														(e.role = (null != (o = b.role) ? o.textContent : void 0) || null),
														(e.jid = (null != (p = b.jid) ? p.textContent : void 0) || null),
														(e.newnick = (null != (q = b.nick) ? q.textContent : void 0) || null);
													break;
												case "status":
													d.attributes.code && e.states.push(d.attributes.code.textContent);
											}
							}
						return e;
					}),
			...
			(b = (function () {
				function a(a) {
					(this.parse = d(this.parse, this)), null != a && this.parse(a);
				}
				return (
					(a.prototype.parse = function (a) {
						var b, c, d, e, f, g, h, i, j, k, l, m, n;
						for (g = a.getElementsByTagName("query")[0].childNodes, this.identities = [], this.features = [], this.x = [], h = 0, k = g.length; k > h; h++)
							switch (((d = g[h]), (c = d.attributes), d.nodeName)) {
								case "identity":
									for (f = {}, i = 0, l = c.length; l > i; i++) (b = c[i]), (f[b.name] = b.textContent);
									this.identities.push(f);
									break;
								case "feature":
									this.features.push(c["var"].textContent);
									break;
								case "x":
									if (((c = d.childNodes[0].attributes), "FORM_TYPE" === !c["var"].textContent || "hidden" === !c.type.textContent)) break;
									for (n = d.childNodes, j = 0, m = n.length; m > j; j++)
										(e = n[j]),
											e.attributes.type ||
												((c = e.attributes),
												this.x.push({
													var: c["var"].textContent,
													label: c.label.textContent || "",
			...
			(a = (function () {
				function a(a, b) {
					(this.room = b),
						...
						(this.admin = d(this.admin, this)),
						(this.owner = d(this.owner, this)),
						(this.revoke = d(this.revoke, this)),
						(this.member = d(this.member, this)),
						(this.ban = d(this.ban, this)),
						(this.modifyAffiliation = d(this.modifyAffiliation, this)),
						(this.deop = d(this.deop, this)),
						(this.op = d(this.op, this)),
						(this.mute = d(this.mute, this)),
						(this.voice = d(this.voice, this)),
						(this.kick = d(this.kick, this)),
						(this.modifyRole = d(this.modifyRole, this)),
						this.update(a);
				}
				return (
					(a.prototype.modifyRole = function (a, b, c, d) {
						return this.room.modifyRole(this.nick, a, b, c, d);
					}),
					(a.prototype.kick = function (a, b, c) {
						return this.room.kick(this.nick, a, b, c);
					}),
					(a.prototype.voice = function (a, b, c) {
						return this.room.voice(this.nick, a, b, c);
					}),
					(a.prototype.mute = function (a, b, c) {
						return this.room.mute(this.nick, a, b, c);
					}),
					(a.prototype.op = function (a, b, c) {
						return this.room.op(this.nick, a, b, c);
					}),
					(a.prototype.deop = function (a, b, c) {
						return this.room.deop(this.nick, a, b, c);
					}),
					(a.prototype.modifyAffiliation = function (a, b, c, d) {
						return this.room.modifyAffiliation(this.jid, a, b, c, d);
					}),
					(a.prototype.ban = function (a, b, c) {
						return this.room.ban(this.jid, a, b, c);
					}),
					(a.prototype.member = function (a, b, c) {
						return this.room.member(this.jid, a, b, c);
					}),
					(a.prototype.revoke = function (a, b, c) {
						return this.room.revoke(this.jid, a, b, c);
					}),
					(a.prototype.owner = function (a, b, c) {
						return this.room.owner(this.jid, a, b, c);
					}),
					(a.prototype.admin = function (a, b, c) {
						return this.room.admin(this.jid, a, b, c);
					}),
					(a.prototype.update = function (a) {
						return (
							(this.nick = a.nick || null),
							(this.affiliation = a.affiliation || null),
							(this.role = a.role || null),
							(this.jid = a.jid || null),
							(this.status = a.status || null),
							(this.show = a.show || null),
							this
						);
					}),
					a
	...
	Strophe.addConnectionPlugin("roster", {
		init: function (a) {
			(this._connection = a), (this._callbacks = []), (this._callbacks_request = []), (items = []), (ver = null);
			var b,
				c = this,
				d = a.connect,
				e = a.attach,
				f = function (d) {
					if (d == Strophe.Status.ATTACHED || d == Strophe.Status.CONNECTED)
						try {
							a.addHandler(c._onReceivePresence.bind(c), null, "presence", null, null, null),
								a.addHandler(c._onReceiveIQ.bind(c), Strophe.NS.ROSTER, "iq", "set", null, null);
						} catch (e) {
							Strophe.error(e);
						}
					null !== b && b.apply(this, arguments);
				};
			(a.connect = function (c, e, g, h, i, j) {
				(b = g), "undefined" == typeof c && (c = null), "undefined" == typeof e && (e = null), (g = f), d.apply(a, [c, e, g, h, i, j]);
			}),
				(a.attach = function (c, d, g, h, i, j, k) {
					(b = h),
						"undefined" == typeof c && (c = null),
						"undefined" == typeof d && (d = null),
						"undefined" == typeof g && (g = null),
						(h = f),
						e.apply(a, [c, d, g, h, i, j, k]);
				}),
		...
		get: function (a, b, c) {
			...
			(this.items = []), this.supportVersioning() && ((d.ver = b || ""), (this.items = c || []));
			var e = $iq({
				type: "get",
				id: this._connection.getUniqueId("roster"),
			}).c("query", d);
			return this._connection.sendIQ(e, this._onReceiveRosterSuccess.bind(this, a), this._onReceiveRosterError.bind(this, a));
		},
		registerCallback: function (a) {
			this._callbacks.push(a);
		},
		registerRequestCallback: function (a) {
			this._callbacks_request.push(a);
		},
		findItem: function (a) {
			for (var b = 0; b < this.items.length; b++) if (this.items[b] && this.items[b].jid == a) return this.items[b];
			return !1;
		},
		removeItem: function (a) {
			for (var b = 0; b < this.items.length; b++) if (this.items[b] && this.items[b].jid == a) return this.items.splice(b, 1), !0;
			return !1;
		},
		...
		unsubscribe: function (a, b) {
			...
			b && "" !== b && c.c("status").t(b), this._connection.send(c);
		},
		...
		add: function (a, b, c, d) {
			for (
				...
				e.c("group").t(c[f]).up();
			this._connection.sendIQ(e, d, d);
		},
		update: function (a, b, c, d) {
			var e = this.findItem(a);
			if (!e) throw "item not found";
			for (
				var f = b || e.name,
					g = c || e.groups,
					h = $iq({
				...
				h.c("group").t(g[i]).up();
			return this._connection.sendIQ(h, d, d);
		},
		remove: function (a, b) {
			var c = this.findItem(a);
			if (!c) throw "item not found";
			var d = $iq({
				type: "set",
			})
				.c("query", {
			...
			this._connection.sendIQ(d, b, b);
		},
		_onReceiveRosterSuccess: function (a, b) {
			this._updateItems(b), this._call_backs(this.items), a(this.items);
		},
		_onReceiveRosterError: function (a, b) {
			a(this.items);
		},
		_onReceivePresence: function (a) {
			var b = a.getAttribute("from"),
				c = Strophe.getBareJidFromJid(b),
				d = this.findItem(c),
				e = a.getAttribute("type");
			if (!d) return "subscribe" === e && this._call_backs_request(c), !0;
			if ("unavailable" == e) delete d.resources[Strophe.getResourceFromJid(b)];
			else {
				if (e) return !0;
				d.resources[Strophe.getResourceFromJid(b)] = {
					show: 0 !== a.getElementsByTagName("show").length ? Strophe.getText(a.getElementsByTagName("show")[0]) : "",
					status: 0 !== a.getElementsByTagName("status").length ? Strophe.getText(a.getElementsByTagName("status")[0]) : "",
					priority: 0 !== a.getElementsByTagName("priority").length ? Strophe.getText(a.getElementsByTagName("priority")[0]) : "",
				};
			}
			return this._call_backs(this.items, d), !0;
		},
		_call_backs_request: function (a) {
			for (var b = 0; b < this._callbacks_request.length; b++) this._callbacks_request[b](a);
		},
		_call_backs: function (a, b) {
			for (var c = 0; c < this._callbacks.length; c++) this._callbacks[c](a, b);
		},
		_onReceiveIQ: function (a) {
			Strophe.info("_onReceiveIQ called = " + this._connection.jid + " ## a=" + a);
			var b = a.getAttribute("id"),
				c = a.getAttribute("from");
			if (c && "" !== c && c != this._connection.jid && c != Strophe.getBareJidFromJid(this._connection.jid)) return !0;
			var d = $iq({
				type: "result",
				id: b,
				from: this._connection.jid,
			});
			return this._connection.send(d), this._updateItems(a), !0;
		},
		_updateItems: function (a) {
			var b = a.getElementsByTagName("query");
			if (0 !== b.length) {
				this.ver = b.item(0).getAttribute("ver");
				var c = this;
				Strophe.forEachChild(b.item(0), "item", function (a) {
					c._updateItem(a);
				});
			}
		},
		_updateItem: function (a) {
			var b = a.getAttribute("jid"),
				c = a.getAttribute("name"),
				d = a.getAttribute("subscription"),
				e = a.getAttribute("ask"),
				f = [];
			return (
				Strophe.forEachChild(a, "group", function (a) {
					f.push(Strophe.getText(a));
				}),
				"remove" == d
					? (this.removeItem(b),
					  void this._call_backs(this.items, {
							jid: b,
							subscription: "remove",
					  }))
					: ((a = this.findItem(b)),
					  a
							? ((a.name = c), (a.subscription = d), (a.ask = e), (a.groups = f))
							: ((a = {
									name: c,
									jid: b,
									subscription: d,
							  ...
							  this.items.push(a)),
					  void this._call_backs(this.items, a))
			);
		},
	}),
	Strophe.addConnectionPlugin("disco", {
		...
		init: function (a) {
			(this._connection = a),
				...
				(this._features = []),
				(this._items = []),
				a.addHandler(this._onDiscoInfo.bind(this), Strophe.NS.DISCO_INFO, "iq", "get", null, null),
				a.addHandler(this._onDiscoItems.bind(this), Strophe.NS.DISCO_ITEMS, "iq", "get", null, null);
		},
		addIdentity: function (a, b, c, d) {
			for (var e = 0; e < this._identities.length; e++)
				if (this._identities[e].category == a && this._identities[e].type == b && this._identities[e].name == c && this._identities[e].lang == d) return !1;
			return (
		...
		addFeature: function (a) {
			for (var b = 0; b < this._features.length; b++) if (this._features[b] == a) return !1;
			return this._features.push(a), !0;
		},
		removeFeature: function (a) {
			for (var b = 0; b < this._features.length; b++) if (this._features[b] === a) return this._features.splice(b, 1), !0;
			return !1;
		},
		...
		info: function (a, b, c, d, e) {
			...
			this._connection.sendIQ(g, c, d, e);
		},
		items: function (a, b, c, d, e) {
			var f = {
				xmlns: Strophe.NS.DISCO_ITEMS,
			};
			b && (f.node = b);
		...
		_buildIQResult: function (a, b) {
			var c = a.getAttribute("id"),
				d = a.getAttribute("from"),
				e = $iq({
					type: "result",
					id: c,
				});
		...
		_onDiscoInfo: function (a) {
			var b = a.getElementsByTagName("query")[0].getAttribute("node"),
				c = {
					xmlns: Strophe.NS.DISCO_INFO,
				};
			b && (c.node = b);
			for (var d = this._buildIQResult(a, c), e = 0; e < this._identities.length; e++) {
				var c = {
					category: this._identities[e].category,
					type: this._identities[e].type,
				};
				this._identities[e].name && (c.name = this._identities[e].name), this._identities[e].lang && (c["xml:lang"] = this._identities[e].lang), d.c("identity", c).up();
			}
			for (var e = 0; e < this._features.length; e++)
				d.c("feature", {
					var: this._features[e],
				}).up();
			return this._connection.send(d.tree()), !0;
		},
		_onDiscoItems: function (a) {
			var b = {
					xmlns: Strophe.NS.DISCO_ITEMS,
				},
				c = a.getElementsByTagName("query")[0].getAttribute("node");
			if (c) {
				b.node = c;
				for (var d = [], e = 0; e < this._items.length; e++)
					if (this._items[e].node == c) {
						d = this._items[e].call_back(a);
						break;
					}
			} else var d = this._items;
			for (var f = this._buildIQResult(a, b), e = 0; e < d.length; e++) {
				var g = {
					jid: d[e].jid,
				};
				d[e].name && (g.name = d[e].name), d[e].node && (g.node = d[e].node), f.c("item", g).up();
			}
			return this._connection.send(f.tree()), !0;
		},
	}),
	Strophe.addConnectionPlugin("caps", {
		...
		_jidVerIndex: {},
		init: function (a) {
			if (((this._connection = a), Strophe.addNamespace("CAPS", "http://jabber.org/protocol/caps"), !this._connection.disco))
				throw "Caps plugin requires the disco plugin to be installed.";
			this._connection.disco.addFeature(Strophe.NS.CAPS), this._connection.addHandler(this._delegateCapabilities.bind(this), Strophe.NS.CAPS);
		},
		...
		generateVer: function () {
			...
			for (
				var a = "", b = this._connection.disco._identities.sort(this._sortIdentities), c = b.length, d = this._connection.disco._features.sort(), e = d.length, f = 0;
				c > f;
				f++
			) {
				var g = b[f];
				a += g.category + "/" + g.type + "/" + g.lang + "/" + g.name + "<";
			}
			for (var f = 0; e > f; f++) a += d[f] + "<";
			return (this._ver = b64_sha1(a)), this._ver;
		},
		getCapabilitiesByJid: function (a) {
			return this._jidVerIndex[a] ? this._knownCapabilities[this._jidVerIndex[a]] : null;
		},
		_delegateCapabilities: function (a) {
			var b = a.getAttribute("from"),
				c = a.querySelector("c"),
				d = c.getAttribute("ver"),
				e = c.getAttribute("node");
			return this._knownCapabilities[d]
				? ((this._jidVerIndex[b] = d), (this._jidVerIndex[b] && !this._jidVerIndex[b] === d) || (this._jidVerIndex[b] = d), !0)
				: this._requestCapabilities(b, e, d);
		},
		_requestCapabilities: function (a, b, c) {
			if (a !== this._connection.jid) {
				var d = this._connection.disco.info(a, b + "#" + c);
				this._connection.addHandler(this._handleDiscoInfoReply.bind(this), Strophe.NS.DISCO_INFO, "iq", "result", d, a);
			}
			return !0;
		},
		_handleDiscoInfoReply: function (a) {
			//console.log(a.querySelector("query").getAttribute("node").split("#"));
			/*  if(a.querySelector("query").getAttribute("node"));
           {
               console.log(a.querySelector("query").getAttribute("node"));
            var b = a.querySelector("query"),
                c = b.getAttribute("node").split("#"),
                d = c[1],
                e = a.getAttribute("from");
            if (this._knownCapabilities[d]) this._jidVerIndex[e] && !this._jidVerIndex[e] === d || (this._jidVerIndex[e] = d);
            else {
                var f = b.childNodes,
                    g = f.length;
                this._knownCapabilities[d] = [];
                for (var h = 0; g > h; h++) {
                    var c = f[h];
                    this._knownCapabilities[d].push({
                        name: c.nodeName,
                        attributes: c.attributes
                    })
                }
                this._jidVerIndex[e] = d
            }
            }*/
			return !1;
		},
		_sortIdentities: function (a, b) {
			return a.category > b.category ? 1 : a.category < b.category ? -1 : a.type > b.type ? 1 : a.type < b.type ? -1 : a.lang > b.lang ? 1 : a.lang < b.lang ? -1 : 0;
		},
	});
		...
		(a.prototype = {
					...
					return "^" == e
						...
						? d.is_array(h)
							? d
									...
									.join("")
							: d.is_object(h)
							? d.render(g, d.create_context(h), c, !0)
							: "function" == typeof h
							? h.call(b, g, function (a) {
			...
			render_tags: function (a, b, c, d) {
				for (
					...
					(i[j] = i[j].replace(g, h, this)), d || this.send(i[j]);
				return d ? i.join("\n") : void 0;
			},
...
!(function (a) {
	var b = Array.prototype.slice,
		c = {
			dict: null,
			load: function (b) {
				null !== this.dict ? a.extend(this.dict, b) : (this.dict = b);
			},
			_: function (a) {
				return (dict = this.dict), dict && dict.hasOwnProperty(a) && (a = dict[a]), (args = b.call(arguments)), (args[0] = a), this.printf.apply(this, args);
			},
			printf: function (c, d) {
				return arguments.length < 2
					? c
					: ((d = a.isArray(d) ? d : b.call(arguments, 1)),
					  c
							.replace(/([^%]|^)%(?:(\d+)\$)?s/g, function (a, b, c) {
								return c ? b + d[parseInt(c) - 1] : b + d.shift();
							})
							.replace(/%%s/g, "%s"));
			},
		};
	(a.fn._t = function (b, d) {
