...
(function (callback) {
	...
	function $file(attrs) {
		return new Strophe.Builder("file", attrs);
	}
			...
			for (i = 0; i < elem.attributes.length; i++) {
				if (elem.attributes[i].nodeName != "_realname") {
					result +=
						...
						"='" +
						elem.attributes[i].value.replace(/&/g, "&amp;").replace(/\'/g, "&apos;").replace(/>/g, "&gt;").replace(/</g, "&lt;") +
						"'";
				}
			}
	...
	Strophe.Builder = function (name, attrs) {
		...
		if (name == "presence" || name == "message" || name == "file" || name == "iq") {
	...
	if (callback) {
		callback(Strophe, $build, $msg, $iq, $pres, $file);
	}
	...
	_processRequest: function (i) {
		...
		if (req.xhr.readyState === 0) {
			...
			try {
				...
				console.log(`XHR open failed. connected >> ${Strophe.Status.CONNFAIL}`);
			...
			if (this._conn.xmlOutput !== Strophe.Connection.prototype.xmlOutput) {
				console.log(`xmlData >> ${req.xmlData}`);
...
Strophe.addConnectionPlugin("roster", {
	...
	_call_backs_request: function (from) {
		for (
			var i = 0;
			i < this._callbacks_request.length;
			i++ // [].forEach my love ...
		) {
